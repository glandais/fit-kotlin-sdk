...
// No `package` declaration, deliberately: everything in this source set is part of the
// JS/TS surface, and Kotlin/JS mirrors the package structure into the generated module.
// A package here would make consumers write `com.garmin.fit.decodeFit(...)` in JavaScript
// and `import { com } from '@glandais/fit-kotlin-sdk'` in TypeScript. See FitJs.kt.

/**
 * The part of the FIT profile a JavaScript caller expects to see as text rather than as
 * numbers: which profile type each field carries, and what each value of those types is
 * called.
 *
 * The Kotlin API has no use for this — the generated `*Mesg` accessors return
 * `Sport.CYCLING`, so the mapping is in the type system already. JavaScript has no such
 * accessors, so `decodeFit` resolves the names here instead and hands back `"cycling"`,
 * matching what `@garmin/fitsdk` and `garmin-fit-sdk` return.
 *
 * Stored as text, not as `mapOf(...)` literals: this is ~1900 field entries and ~4600
 * value entries, and as code they would be tens of thousands of IR nodes in the bundle
 * for data that is never read on a run that decodes nothing. Parsed on first use into
 * maps, once per module load.
 *
 * Line formats (profile names are `[a-z0-9_]`, so the separators cannot occur in them):
 * ```
 * fields:  "<globalMesgNum>|<FieldName>=<type_name>,<SubFieldName>=<type_name>,"
 * values:  "<type_name>|<value>=<value_name>,<value>=<value_name>,"
 * ```
 */
internal object ProfileNames {
    /**
     * The profile type of field [fieldName] on message [mesgNum], or null when the field
     * carries a base type (a plain number or string) and so has no named values.
     *
     * [fieldName] is the name as [com.garmin.fit.Field.fieldName] spells it — upper
     * intercaps, `PositionLat` — and may equally be the name of an active subfield.
     */
    fun typeOf(mesgNum: Int, fieldName: String): String? = fieldTypes[mesgNum]?.get(fieldName)

    /**
     * What the profile calls [value] of type [typeName], or null when the type does not
     * name that exact value.
     *
     * Only exact matches resolve. FIT bit-mask types (`workout_capabilities`) name
     * individual bits, and a value combining two of them means both, not the one that
     * happens to compare equal — so it is left as a number rather than mislabelled.
     */
    fun nameOf(typeName: String, value: Double): String? = typeValues[typeName]?.get(value)

    /** The raw value the profile gives the name [valueName] of type [typeName]. */
    fun valueOf(typeName: String, valueName: String): Double? = typeNames[typeName]?.get(valueName)

    /** Global message number for a message named [mesgName], upper intercaps: `FileId`. */
    fun mesgNumOf(mesgName: String): Int? = mesgNums[mesgName]

    /** Every message name the profile knows, upper intercaps, in profile order. */
    fun mesgNames(): List<String> = mesgNums.keys.toList()

    private val mesgNums: Map<String, Int> by lazy {
        val result = LinkedHashMap<String, Int>(MESG_NAME_LINES.size)
        for (line in MESG_NAME_LINES) {
            val bar = line.indexOf('|')
            result[line.substring(bar + 1)] = line.substring(0, bar).toInt()
        }
        result
    }

    /** [typeValues] inverted, for the encoder: `"cycling"` back to `2.0`. */
    private val typeNames: Map<String, Map<String, Double>> by lazy {
        typeValues.mapValues { (_, values) ->
            val inverted = HashMap<String, Double>(values.size)
            // First wins: a profile type may name the same value twice, and the
            // first spelling is the canonical one nameOf() hands out.
            for ((value, name) in values) inverted.getOrPut(name) { value }
            inverted
        }
    }

    private val fieldTypes: Map<Int, Map<String, String>> by lazy {
        val result = HashMap<Int, Map<String, String>>(FIELD_TYPE_LINES.size)
        for (line in FIELD_TYPE_LINES) {
            val bar = line.indexOf('|')
            result[line.substring(0, bar).toInt()] = parseEntries(line, bar + 1) { it }
        }
        result
    }

    private val typeValues: Map<String, Map<Double, String>> by lazy {
        val result = HashMap<String, Map<Double, String>>(TYPE_VALUE_LINES.size)
        for (line in TYPE_VALUE_LINES) {
            val bar = line.indexOf('|')
            result[line.substring(0, bar)] = parseEntries(line, bar + 1) { parseValue(it) }
        }
        result
    }

    /**
     * A profile value, decimal or hexadecimal: the FIT profile writes range markers as
     * `0xFF00`, and [String.toDouble] does not read those.
     */
    ...
    /** Splits `k=v,k=v,` from [from] onward, mapping each key through [key]. */
    private inline fun <K> parseEntries(line: String, from: Int, key: (String) -> K): Map<K, String> {
        val result = HashMap<K, String>()
        var start = from
        while (start < line.length) {
            val eq = line.indexOf('=', start)
            if (eq < 0) break
            var end = line.indexOf(',', eq + 1)
            if (end < 0) end = line.length
            result[key(line.substring(start, eq))] = line.substring(eq + 1, end)
            start = end + 1
        }
        return result
    }

    private val MESG_NAME_LINES: List<String> = listOf(
        "0|FileId",
        "49|FileCreator",
    ...
    private val FIELD_TYPE_LINES: List<String> = listOf(
        "0|Type=file,Manufacturer=manufacturer,FaveroProduct=favero_product,GarminProduct=garmin_product,TimeCreated=date_time,",
        "49|",
        "162|Timestamp=date_time,SystemTimestamp=date_time,LocalTimestamp=local_date_time,",
        "35|MessageIndex=message_index,",
        "106|Manufacturer=manufacturer,FaveroProduct=favero_product,GarminProduct=garmin_product,",
        "1|Sports=sport_bits_0,WorkoutsSupported=workout_capabilities,ConnectivitySupported=connectivity_capabilities,",
        "37|MessageIndex=message_index,Type=file,Flags=file_flags,",
        "38|MessageIndex=message_index,File=file,MesgNum=mesg_num,CountType=mesg_count,",
        "39|MessageIndex=message_index,File=file,MesgNum=mesg_num,",
        "2|TimeMode=time_mode,BacklightMode=backlight_mode,ClockTime=date_time,DateMode=date_mode,DisplayOrientation=display_orientation,MountingSide=side,AutoSyncFrequency=auto_sync_frequency,AutoActivityDetect=auto_activity_detect,SmartNotificationDisplayOrientation=display_orientation,TapInterface=switch,TapSensitivity=tap_sensitivity,",
        "3|MessageIndex=message_index,Gender=gender,Language=language,ElevSetting=display_measure,WeightSetting=display_measure,HrSetting=display_heart,SpeedSetting=display_measure,DistSetting=display_measure,PowerSetting=display_power,ActivityClass=activity_class,PositionSetting=display_position,TemperatureSetting=display_measure,LocalId=user_local_id,WakeTime=localtime_into_day,SleepTime=localtime_into_day,HeightSetting=display_measure,DepthSetting=display_measure,",
        "4|MessageIndex=message_index,",
        "5|MessageIndex=message_index,",
        "6|MessageIndex=message_index,Sport=sport,SubSport=sub_sport,",
        "127|",
        "159|MessageIndex=message_index,Mode=watchface_mode,DigitalLayout=digital_watchface_layout,AnalogLayout=analog_watchface_layout,",
        "188|Timestamp=date_time,Enabled=switch,",
        "216|Timestamp=date_time,ReferenceMesg=mesg_num,ReferenceIndex=message_index,HrCalcType=hr_zone_calc,PwrCalcType=pwr_zone_calc,",
        "7|HrCalcType=hr_zone_calc,PwrCalcType=pwr_zone_calc,",
        "12|Sport=sport,SubSport=sub_sport,",
        "8|MessageIndex=message_index,",
        "53|MessageIndex=message_index,",
        "131|MessageIndex=message_index,",
        "9|MessageIndex=message_index,",
        "10|MessageIndex=message_index,",
        "13|",
        "258|Timestamp=date_time,MessageIndex=message_index,Model=tissue_model_type,WaterType=water_type,BacklightMode=dive_backlight_mode,BacklightTimeout=backlight_timeout,HeartRateSourceType=source_type,HeartRateAntplusDeviceType=antplus_device_type,HeartRateLocalDeviceType=local_device_type,TravelGas=message_index,CcrLowSetpointSwitchMode=ccr_setpoint_switch_mode,CcrHighSetpointSwitchMode=ccr_setpoint_switch_mode,GasConsumptionDisplay=gas_consumption_rate_type,DiveSounds=tone,NoFlyTimeMode=no_fly_time_mode,",
        "262|MessageIndex=message_index,AlarmType=dive_alarm_type,Sound=tone,DiveTypes=sub_sport,",
        "393|MessageIndex=message_index,AlarmType=dive_alarm_type,Sound=tone,DiveTypes=sub_sport,",
        "259|MessageIndex=message_index,Status=dive_gas_status,Mode=dive_gas_mode,",
        "15|MessageIndex=message_index,Sport=sport,SubSport=sub_sport,StartDate=date_time,EndDate=date_time,Type=goal,Recurrence=goal_recurrence,Source=goal_source,",
        "34|Timestamp=date_time,Type=activity,Event=event,EventType=event_type,LocalTimestamp=local_date_time,",
        "18|MessageIndex=message_index,Timestamp=date_time,Event=event,EventType=event_type,StartTime=date_time,Sport=sport,SubSport=sub_sport,Trigger=session_trigger,LeftRightBalance=left_right_balance_100,SwimStroke=swim_stroke,PoolLengthUnit=display_measure,",
        "19|MessageIndex=message_index,Timestamp=date_time,Event=event,EventType=event_type,StartTime=date_time,Intensity=intensity,LapTrigger=lap_trigger,Sport=sport,LeftRightBalance=left_right_balance_100,SwimStroke=swim_stroke,SubSport=sub_sport,WktStepIndex=message_index,",
        "101|MessageIndex=message_index,Timestamp=date_time,Event=event,EventType=event_type,StartTime=date_time,SwimStroke=swim_stroke,LengthType=length_type,",
        "20|Timestamp=date_time,LeftRightBalance=left_right_balance,ActivityType=activity_type,StrokeType=stroke_type,DeviceIndex=device_index,",
        "21|Timestamp=date_time,Event=event,EventType=event_type,TimerTrigger=timer_trigger,CoursePointIndex=message_index,FitnessEquipmentState=fitness_equipment_state,RiderPosition=rider_position_type,CommTimeout=comm_timeout_type,DiveAlert=dive_alert,DeviceIndex=device_index,ActivityType=activity_type,StartTimestamp=date_time,AutoActivityDetectStartTimestamp=date_time,RadarThreatLevelMax=radar_threat_level_type,",
        "23|Timestamp=date_time,DeviceIndex=device_index,BleDeviceType=ble_device_type,AntplusDeviceType=antplus_device_type,LocalDeviceType=local_device_type,Manufacturer=manufacturer,FaveroProduct=favero_product,GarminProduct=garmin_product,BatteryStatus=battery_status,SensorPosition=body_location,AntNetwork=ant_network,SourceType=source_type,",
        "375|Timestamp=date_time,DeviceIndex=device_index,BatteryStatus=battery_status,",
        "72|Timestamp=date_time,Type=file,Manufacturer=manufacturer,FaveroProduct=favero_product,GarminProduct=garmin_product,TimeCreated=date_time,",
        "128|Timestamp=date_time,WeatherReport=weather_report,Condition=weather_status,ObservedAtTime=date_time,DayOfWeek=day_of_week,",
        "129|Timestamp=date_time,IssueTime=date_time,ExpireTime=date_time,Severity=weather_severity,Type=weather_severe_type,",
        "160|Timestamp=date_time,UtcTimestamp=date_time,",
        "161|Timestamp=date_time,CameraEventType=camera_event_type,CameraOrientation=camera_orientation_type,",
        "164|Timestamp=date_time,",
        "165|Timestamp=date_time,",
        "208|Timestamp=date_time,",
        "209|Timestamp=date_time,",
        "167|Timestamp=date_time,SensorType=sensor_type,",
        "210|Timestamp=date_time,SensorType=sensor_type,",
        "169|Timestamp=date_time,",
        "174|Timestamp=date_time,StartTimestamp=date_time,",
        "177|Timestamp=date_time,",
        "178|Timestamp=date_time,Stage=attitude_stage,Validity=attitude_validity,",
        "184|",
        "185|MessageIndex=message_index,",
        "186|MessageIndex=message_index,",
        "187|StartTimestamp=date_time,EndTimestamp=date_time,",
        "225|Timestamp=date_time,SetType=set_type,StartTime=date_time,Category=exercise_category,WeightDisplayUnit=fit_base_unit,MessageIndex=message_index,WktStepIndex=message_index,",
        "285|Timestamp=date_time,",
        "312|MessageIndex=message_index,SplitType=split_type,StartTime=date_time,EndTime=date_time,",
        "313|MessageIndex=message_index,SplitType=split_type,",
        "317|Timestamp=date_time,ClimbProEvent=climb_pro_event,",
        "206|FitBaseTypeId=fit_base_type,FitBaseUnitId=fit_base_unit,NativeMesgNum=mesg_num,",
        "207|ManufacturerId=manufacturer,",
        "31|Sport=sport,Capabilities=course_capabilities,SubSport=sub_sport,",
        "32|MessageIndex=message_index,Timestamp=date_time,Type=course_point,",
        "148|Sport=sport,DeleteStatus=segment_delete_status,SelectionType=segment_selection_type,",
        "149|MessageIndex=message_index,Type=segment_leaderboard_type,",
        "150|MessageIndex=message_index,",
        "142|MessageIndex=message_index,Timestamp=date_time,Event=event,EventType=event_type,StartTime=date_time,Sport=sport,LeftRightBalance=left_right_balance_100,SubSport=sub_sport,WktStepIndex=message_index,SportEvent=sport_event,Status=segment_lap_status,Manufacturer=manufacturer,",
        "151|MessageIndex=message_index,LeaderType=segment_leaderboard_type,",
        "26|MessageIndex=message_index,Sport=sport,Capabilities=workout_capabilities,SubSport=sub_sport,PoolLengthUnit=display_measure,",
        "158|MessageIndex=message_index,Sport=sport,SubSport=sub_sport,PoolLengthUnit=display_measure,",
        "27|MessageIndex=message_index,DurationType=wkt_step_duration,DurationHr=workout_hr,DurationPower=workout_power,TargetType=wkt_step_target,RepeatHr=workout_hr,RepeatPower=workout_power,TargetStrokeType=swim_stroke,CustomTargetHeartRateLow=workout_hr,CustomTargetPowerLow=workout_power,CustomTargetHeartRateHigh=workout_hr,CustomTargetPowerHigh=workout_power,Intensity=intensity,Equipment=workout_equipment,ExerciseCategory=exercise_category,WeightDisplayUnit=fit_base_unit,SecondaryTargetType=wkt_step_target,SecondaryTargetStrokeType=swim_stroke,SecondaryCustomTargetHeartRateLow=workout_hr,SecondaryCustomTargetPowerLow=workout_power,SecondaryCustomTargetHeartRateHigh=workout_hr,SecondaryCustomTargetPowerHigh=workout_power,",
        "264|MessageIndex=message_index,ExerciseCategory=exercise_category,",
        "28|Manufacturer=manufacturer,FaveroProduct=favero_product,GarminProduct=garmin_product,TimeCreated=date_time,Type=schedule,ScheduledTime=local_date_time,",
        "33|MessageIndex=message_index,Timestamp=date_time,Sport=sport,",
        "30|Timestamp=date_time,Weight=weight,UserProfileIndex=message_index,",
        "51|Timestamp=date_time,HeartRateType=hr_type,Status=bp_status,UserProfileIndex=message_index,",
        "103|Timestamp=date_time,LocalTimestamp=local_date_time,ActivityType=activity_type,",
        "55|Timestamp=date_time,DeviceIndex=device_index,ActivityType=activity_type,ActivitySubtype=activity_subtype,ActivityLevel=activity_level,LocalTimestamp=local_date_time,",
        "211|Timestamp=date_time,",
        "269|Timestamp=date_time,Mode=spo2_measurement_type,",
        "132|Timestamp=date_time,",
        "227|StressLevelTime=date_time,",
        "229|UpdateTime=date_time,Sport=sport,SubSport=sub_sport,MaxMetCategory=max_met_category,HrSource=max_met_heart_rate_source,SpeedSource=max_met_speed_source,",
        "314|Timestamp=date_time,",
        "315|Timestamp=date_time,",
        "302|Timestamp=date_time,",
        "376|Timestamp=date_time,",
        "304|Timestamp=date_time,",
        "305|Timestamp=date_time,",
        "306|Timestamp=date_time,",
        "307|Timestamp=date_time,",
        "308|Timestamp=date_time,",
        "389|Timestamp=date_time,",
        "409|Timestamp=date_time,",
        "145|MesgNum=mesg_num,ParentIndex=message_index,",
        "275|Timestamp=date_time,SleepLevel=sleep_level,",
        "82|DeviceIndex=device_index,",
        "80|Timestamp=date_time,",
        "81|Timestamp=date_time,",
        "200|Layout=exd_layout,",
        "201|DisplayType=exd_display_type,",
        "202|DataUnits=exd_data_units,Qualifier=exd_qualifiers,Descriptor=exd_descriptors,",
        "268|Timestamp=date_time,ReferenceMesg=mesg_num,ReferenceIndex=message_index,",
        "289|Timestamp=date_time,",
        "78|",
        "290|Timestamp=date_time,",
        "370|Timestamp=date_time,Status=hrv_status,",
        "371|Timestamp=date_time,",
        "372|Timestamp=date_time,",
        "297|Timestamp=date_time,",
        "387|Timestamp=date_time,ProjectileType=projectile_type,",
        "388|Timestamp=date_time,",
        "319|Timestamp=date_time,Sensor=ant_channel_id,",
        "323|Timestamp=date_time,Sensor=ant_channel_id,",
        "346|",
        "470|MessageIndex=message_index,Timestamp=date_time,Severity=sleep_disruption_severity,",
        "471|Timestamp=date_time,Severity=sleep_disruption_severity,",
        "412|MessageIndex=message_index,Timestamp=date_time,StartTime=date_time,EndTime=date_time,Feedback=nap_period_feedback,Source=nap_source,UpdateTimestamp=date_time,",
        "398|Timestamp=date_time,LocalTimestamp=local_date_time,",
        "105|",
    )

    private val TYPE_VALUE_LINES: List<String> = listOf(
        "file|1=device,2=settings,3=sport,4=activity,5=workout,6=course,7=schedules,9=weight,10=totals,11=goals,14=blood_pressure,15=monitoring_a,20=activity_summary,28=monitoring_daily,32=monitoring_b,34=segment,35=segment_list,40=exd_configuration,0xF7=mfg_range_min,0xFE=mfg_range_max,",
        "mesg_num|0=file_id,1=capabilities,2=device_settings,3=user_profile,4=hrm_profile,5=sdm_profile,6=bike_profile,7=zones_target,8=hr_zone,9=power_zone,10=met_zone,12=sport,13=training_settings,15=goal,18=session,19=lap,20=record,21=event,23=device_info,26=workout,27=workout_step,28=schedule,30=weight_scale,31=course,32=course_point,33=totals,34=activity,35=software,37=file_capabilities,38=mesg_capabilities,39=field_capabilities,49=file_creator,51=blood_pressure,53=speed_zone,55=monitoring,72=training_file,78=hrv,80=ant_rx,81=ant_tx,82=ant_channel_id,101=length,103=monitoring_info,105=pad,106=slave_device,127=connectivity,128=weather_conditions,129=weather_alert,131=cadence_zone,132=hr,142=segment_lap,145=memo_glob,148=segment_id,149=segment_leaderboard_entry,150=segment_point,151=segment_file,158=workout_session,159=watchface_settings,160=gps_metadata,161=camera_event,162=timestamp_correlation,164=gyroscope_data,165=accelerometer_data,167=three_d_sensor_calibration,169=video_frame,174=obdii_data,177=nmea_sentence,178=aviation_attitude,184=video,185=video_title,186=video_description,187=video_clip,188=ohr_settings,200=exd_screen_configuration,201=exd_data_field_configuration,202=exd_data_concept_configuration,206=field_description,207=developer_data_id,208=magnetometer_data,209=barometer_data,210=one_d_sensor_calibration,211=monitoring_hr_data,216=time_in_zone,225=set,227=stress_level,229=max_met_data,258=dive_settings,259=dive_gas,262=dive_alarm,264=exercise_title,268=dive_summary,269=spo2_data,275=sleep_level,285=jump,289=aad_accel_features,290=beat_intervals,297=respiration_rate,302=hsa_accelerometer_data,304=hsa_step_data,305=hsa_spo2_data,306=hsa_stress_data,307=hsa_respiration_data,308=hsa_heart_rate_data,312=split,313=split_summary,314=hsa_body_battery_data,315=hsa_event,317=climb_pro,319=tank_update,323=tank_summary,346=sleep_assessment,370=hrv_status_summary,371=hrv_value,372=raw_bbi,375=device_aux_battery_info,376=hsa_gyroscope_data,387=chrono_shot_session,388=chrono_shot_data,389=hsa_configuration_data,393=dive_apnea_alarm,398=skin_temp_overnight,409=hsa_wrist_temperature_data,412=nap_event,470=sleep_disruption_severity_period,471=sleep_disruption_overnight_severity,0xFF00=mfg_range_min,0xFFFE=mfg_range_max,",
        "checksum|0=clear,1=ok,",
        "file_flags|0x02=read,0x04=write,0x08=erase,",
        "mesg_count|0=num_per_file,1=max_per_file,2=max_per_file_type,",
        "date_time|0x10000000=min,",
        "local_date_time|0x10000000=min,",
        "message_index|0x8000=selected,0x7000=reserved,0x0FFF=mask,",
        "device_index|0=creator,",
        "gender|0=female,1=male,",
        "language|0=english,1=french,2=italian,3=german,4=spanish,5=croatian,6=czech,7=danish,8=dutch,9=finnish,10=greek,11=hungarian,12=norwegian,13=polish,14=portuguese,15=slovakian,16=slovenian,17=swedish,18=russian,19=turkish,20=latvian,21=ukrainian,22=arabic,23=farsi,24=bulgarian,25=romanian,26=chinese,27=japanese,28=korean,29=taiwanese,30=thai,31=hebrew,32=brazilian_portuguese,33=indonesian,34=malaysian,35=vietnamese,36=burmese,37=mongolian,254=custom,",
        "language_bits_0|0x01=english,0x02=french,0x04=italian,0x08=german,0x10=spanish,0x20=croatian,0x40=czech,0x80=danish,",
        "language_bits_1|0x01=dutch,0x02=finnish,0x04=greek,0x08=hungarian,0x10=norwegian,0x20=polish,0x40=portuguese,0x80=slovakian,",
        "language_bits_2|0x01=slovenian,0x02=swedish,0x04=russian,0x08=turkish,0x10=latvian,0x20=ukrainian,0x40=arabic,0x80=farsi,",
        "language_bits_3|0x01=bulgarian,0x02=romanian,0x04=chinese,0x08=japanese,0x10=korean,0x20=taiwanese,0x40=thai,0x80=hebrew,",
        "language_bits_4|0x01=brazilian_portuguese,0x02=indonesian,0x04=malaysian,0x08=vietnamese,0x10=burmese,0x20=mongolian,",
        "time_zone|0=almaty,1=bangkok,2=bombay,3=brasilia,4=cairo,5=cape_verde_is,6=darwin,7=eniwetok,8=fiji,9=hong_kong,10=islamabad,11=kabul,12=magadan,13=mid_atlantic,14=moscow,15=muscat,16=newfoundland,17=samoa,18=sydney,19=tehran,20=tokyo,21=us_alaska,22=us_atlantic,23=us_central,24=us_eastern,25=us_hawaii,26=us_mountain,27=us_pacific,28=other,29=auckland,30=kathmandu,31=europe_western_wet,32=europe_central_cet,33=europe_eastern_eet,34=jakarta,35=perth,36=adelaide,37=brisbane,38=tasmania,39=iceland,40=amsterdam,41=athens,42=barcelona,43=berlin,44=brussels,45=budapest,46=copenhagen,47=dublin,48=helsinki,49=lisbon,50=london,51=madrid,52=munich,53=oslo,54=paris,55=prague,56=reykjavik,57=rome,58=stockholm,59=vienna,60=warsaw,61=zurich,62=quebec,63=ontario,64=manitoba,65=saskatchewan,66=alberta,67=british_columbia,68=boise,69=boston,70=chicago,71=dallas,72=denver,73=kansas_city,74=las_vegas,75=los_angeles,76=miami,77=minneapolis,78=new_york,79=new_orleans,80=phoenix,81=santa_fe,82=seattle,83=washington_dc,84=us_arizona,85=chita,86=ekaterinburg,87=irkutsk,88=kaliningrad,89=krasnoyarsk,90=novosibirsk,91=petropavlovsk_kamchatskiy,92=samara,93=vladivostok,94=mexico_central,95=mexico_mountain,96=mexico_pacific,97=cape_town,98=winkhoek,99=lagos,100=riyahd,101=venezuela,102=australia_lh,103=santiago,253=manual,254=automatic,",
        "display_measure|0=metric,1=statute,2=nautical,",
        "display_heart|0=bpm,1=max,2=reserve,",
        "display_power|0=watts,1=percent_ftp,",
        "display_position|0=degree,1=degree_minute,2=degree_minute_second,3=austrian_grid,4=british_grid,5=dutch_grid,6=hungarian_grid,7=finnish_grid,8=german_grid,9=icelandic_grid,10=indonesian_equatorial,11=indonesian_irian,12=indonesian_southern,13=india_zone_0,14=india_zone_IA,15=india_zone_IB,16=india_zone_IIA,17=india_zone_IIB,18=india_zone_IIIA,19=india_zone_IIIB,20=india_zone_IVA,21=india_zone_IVB,22=irish_transverse,23=irish_grid,24=loran,25=maidenhead_grid,26=mgrs_grid,27=new_zealand_grid,28=new_zealand_transverse,29=qatar_grid,30=modified_swedish_grid,31=swedish_grid,32=south_african_grid,33=swiss_grid,34=taiwan_grid,35=united_states_grid,36=utm_ups_grid,37=west_malayan,38=borneo_rso,39=estonian_grid,40=latvian_grid,41=swedish_ref_99_grid,",
        "switch|0=off,1=on,2=auto,",
        "sport|0=generic,1=running,2=cycling,3=transition,4=fitness_equipment,5=swimming,6=basketball,7=soccer,8=tennis,9=american_football,10=training,11=walking,12=cross_country_skiing,13=alpine_skiing,14=snowboarding,15=rowing,16=mountaineering,17=hiking,18=multisport,19=paddling,20=flying,21=e_biking,22=motorcycling,23=boating,24=driving,25=golf,26=hang_gliding,27=horseback_riding,28=hunting,29=fishing,30=inline_skating,31=rock_climbing,32=sailing,33=ice_skating,34=sky_diving,35=snowshoeing,36=snowmobiling,37=stand_up_paddleboarding,38=surfing,39=wakeboarding,40=water_skiing,41=kayaking,42=rafting,43=windsurfing,44=kitesurfing,45=tactical,46=jumpmaster,47=boxing,48=floor_climbing,49=baseball,53=diving,56=shooting,58=winter_sport,59=grinding,62=hiit,63=video_gaming,64=racket,65=wheelchair_push_walk,66=wheelchair_push_run,67=meditation,68=para_sport,69=disc_golf,70=team_sport,71=cricket,72=rugby,73=hockey,74=lacrosse,75=volleyball,76=water_tubing,77=wakesurfing,78=water_sport,79=archery,80=mixed_martial_arts,81=motor_sports,82=snorkeling,83=dance,84=jump_rope,85=pool_apnea,86=mobility,87=geocaching,88=canoeing,254=all,",
        "sport_bits_0|0x01=generic,0x02=running,0x04=cycling,0x08=transition,0x10=fitness_equipment,0x20=swimming,0x40=basketball,0x80=soccer,",
        "sport_bits_1|0x01=tennis,0x02=american_football,0x04=training,0x08=walking,0x10=cross_country_skiing,0x20=alpine_skiing,0x40=snowboarding,0x80=rowing,",
        "sport_bits_2|0x01=mountaineering,0x02=hiking,0x04=multisport,0x08=paddling,0x10=flying,0x20=e_biking,0x40=motorcycling,0x80=boating,",
        "sport_bits_3|0x01=driving,0x02=golf,0x04=hang_gliding,0x08=horseback_riding,0x10=hunting,0x20=fishing,0x40=inline_skating,0x80=rock_climbing,",
        "sport_bits_4|0x01=sailing,0x02=ice_skating,0x04=sky_diving,0x08=snowshoeing,0x10=snowmobiling,0x20=stand_up_paddleboarding,0x40=surfing,0x80=wakeboarding,",
        "sport_bits_5|0x01=water_skiing,0x02=kayaking,0x04=rafting,0x08=windsurfing,0x10=kitesurfing,0x20=tactical,0x40=jumpmaster,0x80=boxing,",
        "sport_bits_6|0x01=floor_climbing,",
        "sub_sport|0=generic,1=treadmill,2=street,3=trail,4=track,5=spin,6=indoor_cycling,7=road,8=mountain,9=downhill,10=recumbent,11=cyclocross,12=hand_cycling,13=track_cycling,14=indoor_rowing,15=elliptical,16=stair_climbing,17=lap_swimming,18=open_water,19=flexibility_training,20=strength_training,21=warm_up,22=match,23=exercise,24=challenge,25=indoor_skiing,26=cardio_training,27=indoor_walking,28=e_bike_fitness,29=bmx,30=casual_walking,31=speed_walking,32=bike_to_run_transition,33=run_to_bike_transition,34=swim_to_bike_transition,35=atv,36=motocross,37=backcountry,38=resort,39=rc_drone,40=wingsuit,41=whitewater,42=skate_skiing,43=yoga,44=pilates,45=indoor_running,46=gravel_cycling,47=e_bike_mountain,48=commuting,49=mixed_surface,50=navigate,51=track_me,52=map,53=single_gas_diving,54=multi_gas_diving,55=gauge_diving,56=apnea_diving,57=apnea_hunting,58=virtual_activity,59=obstacle,62=breathing,63=ccr_diving,65=sail_race,66=expedition,67=ultra,68=indoor_climbing,69=bouldering,70=hiit,71=indoor_grinding,72=hunting_with_dogs,73=amrap,74=emom,75=tabata,77=esport,78=triathlon,79=duathlon,80=brick,81=swim_run,82=adventure_race,83=trucker_workout,84=pickleball,85=padel,86=indoor_wheelchair_walk,87=indoor_wheelchair_run,88=indoor_hand_cycling,90=field,91=ice,92=ultimate,93=platform,94=squash,95=badminton,96=racquetball,97=table_tennis,98=overland,99=trolling_motor,110=fly_canopy,111=fly_paraglide,112=fly_paramotor,113=fly_pressurized,114=fly_navigate,115=fly_timer,116=fly_altimeter,117=fly_wx,118=fly_vfr,119=fly_ifr,121=dynamic_apnea,123=enduro,124=rucking,125=rally,126=pool_triathlon,127=e_bike_enduro,254=all,",
        "sport_event|0=uncategorized,1=geocaching,2=fitness,3=recreation,4=race,5=special_event,6=training,7=transportation,8=touring,",
        ...
        "workout_capabilities|0x00000001=interval,0x00000002=custom,0x00000004=fitness_equipment,0x00000008=firstbeat,0x00000010=new_leaf,0x00000020=tcx,0x00000080=speed,0x00000100=heart_rate,0x00000200=distance,0x00000400=cadence,0x00000800=power,0x00001000=grade,0x00002000=resistance,0x00004000=protected,",
        "battery_status|1=new,2=good,3=ok,4=low,5=critical,6=charging,7=unknown,",
        "hr_type|0=normal,1=irregular,",
        "course_capabilities|0x00000001=processed,0x00000002=valid,0x00000004=time,0x00000008=distance,0x00000010=position,0x00000020=heart_rate,0x00000040=power,0x00000080=cadence,0x00000100=training,0x00000200=navigation,0x00000400=bikeway,0x00001000=aviation,",
        "weight|0xFFFE=calculating,",
        "workout_hr|100=bpm_offset,",
        "workout_power|1000=watts_offset,",
        "bp_status|0=no_error,1=error_incomplete_data,2=error_no_measurement,3=error_data_out_of_range,4=error_irregular_heart_rate,",
        "user_local_id|0x0000=local_min,0x000F=local_max,0x0010=stationary_min,0x00FF=stationary_max,0x0100=portable_min,0xFFFE=portable_max,",
        "swim_stroke|0=freestyle,1=backstroke,2=breaststroke,3=butterfly,4=drill,5=mixed,6=im,7=im_by_round,8=rimo,",
        "activity_type|0=generic,1=running,2=cycling,3=transition,4=fitness_equipment,5=swimming,6=walking,8=sedentary,13=wheelchair_pushing,254=all,",
        "activity_subtype|0=generic,1=treadmill,2=street,3=trail,4=track,5=spin,6=indoor_cycling,7=road,8=mountain,9=downhill,10=recumbent,11=cyclocross,12=hand_cycling,13=track_cycling,14=indoor_rowing,15=elliptical,16=stair_climbing,17=lap_swimming,18=open_water,254=all,",
        ...
        "connectivity_capabilities|0x00000001=bluetooth,0x00000002=bluetooth_le,0x00000004=ant,0x00000008=activity_upload,0x00000010=course_download,0x00000020=workout_download,0x00000040=live_track,0x00000080=weather_conditions,0x00000100=weather_alerts,0x00000200=gps_ephemeris_download,0x00000400=explicit_archive,0x00000800=setup_incomplete,0x00001000=continue_sync_after_software_update,0x00002000=connect_iq_app_download,0x00004000=golf_course_download,0x00008000=device_initiates_sync,0x00010000=connect_iq_watch_app_download,0x00020000=connect_iq_widget_download,0x00040000=connect_iq_watch_face_download,0x00080000=connect_iq_data_field_download,0x00100000=connect_iq_app_managment,0x00200000=swing_sensor,0x00400000=swing_sensor_remote,0x00800000=incident_detection,0x01000000=audio_prompts,0x02000000=wifi_verification,0x04000000=true_up,0x08000000=find_my_watch,0x10000000=remote_manual_sync,0x20000000=live_track_auto_start,0x40000000=live_track_messaging,0x80000000=instant_input,",
        "weather_report|0=current,1=forecast,2=daily_forecast,",
        "weather_status|0=clear,1=partly_cloudy,2=mostly_cloudy,3=rain,4=snow,5=windy,6=thunderstorms,7=wintry_mix,8=fog,11=hazy,12=hail,13=scattered_showers,14=scattered_thunderstorms,15=unknown_precipitation,16=light_rain,17=heavy_rain,18=light_snow,19=heavy_snow,20=light_rain_snow,21=heavy_rain_snow,22=cloudy,",
        "weather_severity|0=unknown,1=warning,2=watch,3=advisory,4=statement,",
        "weather_severe_type|0=unspecified,1=tornado,2=tsunami,3=hurricane,4=extreme_wind,5=typhoon,6=inland_hurricane,7=hurricane_force_wind,8=waterspout,9=severe_thunderstorm,10=wreckhouse_winds,11=les_suetes_wind,12=avalanche,13=flash_flood,14=tropical_storm,15=inland_tropical_storm,16=blizzard,17=ice_storm,18=freezing_rain,19=debris_flow,20=flash_freeze,21=dust_storm,22=high_wind,23=winter_storm,24=heavy_freezing_spray,25=extreme_cold,26=wind_chill,27=cold_wave,28=heavy_snow_alert,29=lake_effect_blowing_snow,30=snow_squall,31=lake_effect_snow,32=winter_weather,33=sleet,34=snowfall,35=snow_and_blowing_snow,36=blowing_snow,37=snow_alert,38=arctic_outflow,39=freezing_drizzle,40=storm,41=storm_surge,42=rainfall,43=areal_flood,44=coastal_flood,45=lakeshore_flood,46=excessive_heat,47=heat,48=weather,49=high_heat_and_humidity,50=humidex_and_health,51=humidex,52=gale,53=freezing_spray,54=special_marine,55=squall,56=strong_wind,57=lake_wind,58=marine_weather,59=wind,60=small_craft_hazardous_seas,61=hazardous_seas,62=small_craft,63=small_craft_winds,64=small_craft_rough_bar,65=high_water_level,66=ashfall,67=freezing_fog,68=dense_fog,69=dense_smoke,70=blowing_dust,71=hard_freeze,72=freeze,73=frost,74=fire_weather,75=flood,76=rip_tide,77=high_surf,78=smog,79=air_quality,80=brisk_wind,81=air_stagnation,82=low_water,83=hydrological,84=special_weather,",
        "time_into_day|",
        "localtime_into_day|",
        "stroke_type|0=no_event,1=other,2=serve,3=forehand,4=backhand,5=smash,",
        "body_location|0=left_leg,1=left_calf,2=left_shin,3=left_hamstring,4=left_quad,5=left_glute,6=right_leg,7=right_calf,8=right_shin,9=right_hamstring,10=right_quad,11=right_glute,12=torso_back,13=left_lower_back,14=left_upper_back,15=right_lower_back,16=right_upper_back,17=torso_front,18=left_abdomen,19=left_chest,20=right_abdomen,21=right_chest,22=left_arm,23=left_shoulder,24=left_bicep,25=left_tricep,26=left_brachioradialis,27=left_forearm_extensors,28=right_arm,29=right_shoulder,30=right_bicep,31=right_tricep,32=right_brachioradialis,33=right_forearm_extensors,34=neck,35=throat,36=waist_mid_back,37=waist_front,38=waist_left,39=waist_right,",
        "segment_lap_status|0=end,1=fail,",
        "segment_leaderboard_type|0=overall,1=personal_best,2=connections,3=group,4=challenger,5=kom,6=qom,7=pr,8=goal,9=carrot,10=club_leader,11=rival,12=last,13=recent_best,14=course_record,",
        "segment_delete_status|0=do_not_delete,1=delete_one,2=delete_all,",
        "segment_selection_type|0=starred,1=suggested,",
        "source_type|0=ant,1=antplus,2=bluetooth,3=bluetooth_low_energy,4=wifi,5=local,",
        "local_device_type|0=gps,1=glonass,2=gps_glonass,3=accelerometer,4=barometer,5=temperature,10=whr,12=sensor_hub,",
        "ble_device_type|0=connected_gps,1=heart_rate,2=bike_power,3=bike_speed_cadence,4=bike_speed,5=bike_cadence,6=footpod,7=bike_trainer,",
        "ant_channel_id|0xF0000000=ant_extended_device_number_upper_nibble,0x0F000000=ant_transmission_type_lower_nibble,0x00FF0000=ant_device_type,0x0000FFFF=ant_device_number,",
        "display_orientation|0=auto,1=portrait,2=landscape,3=portrait_flipped,4=landscape_flipped,",
        "workout_equipment|0=none,1=swim_fins,2=swim_kickboard,3=swim_paddles,4=swim_pull_buoy,5=swim_snorkel,",
        "watchface_mode|0=digital,1=analog,2=connect_iq,3=disabled,",
        "digital_watchface_layout|0=traditional,1=modern,2=bold,",
        "analog_watchface_layout|0=minimal,1=traditional,2=modern,",
        "rider_position_type|0=seated,1=standing,2=transition_to_seated,3=transition_to_standing,",
        "power_phase_type|0=power_phase_start_angle,1=power_phase_end_angle,2=power_phase_arc_length,3=power_phase_center,",
        "camera_event_type|0=video_start,1=video_split,2=video_end,3=photo_taken,4=video_second_stream_start,5=video_second_stream_split,6=video_second_stream_end,7=video_split_start,8=video_second_stream_split_start,11=video_pause,12=video_second_stream_pause,13=video_resume,14=video_second_stream_resume,",
        "sensor_type|0=accelerometer,1=gyroscope,2=compass,3=barometer,",
        "bike_light_network_config_type|0=auto,4=individual,5=high_visibility,6=trail,",
        "comm_timeout_type|0=wildcard_pairing_timeout,1=pairing_timeout,2=connection_lost,3=connection_timeout,",
        "camera_orientation_type|0=camera_orientation_0,1=camera_orientation_90,2=camera_orientation_180,3=camera_orientation_270,",
        "attitude_stage|0=failed,1=aligning,2=degraded,3=valid,",
        "attitude_validity|0x0001=track_angle_heading_valid,0x0002=pitch_valid,0x0004=roll_valid,0x0008=lateral_body_accel_valid,0x0010=normal_body_accel_valid,0x0020=turn_rate_valid,0x0040=hw_fail,0x0080=mag_invalid,0x0100=no_gps,0x0200=gps_invalid,0x0400=solution_coasting,0x0800=true_track_angle,0x1000=magnetic_heading,",
        "auto_sync_frequency|0=never,1=occasionally,2=frequent,3=once_a_day,4=remote,",
        "exd_layout|0=full_screen,1=half_vertical,2=half_horizontal,3=half_vertical_right_split,4=half_horizontal_bottom_split,5=full_quarter_split,6=half_vertical_left_split,7=half_horizontal_top_split,8=dynamic,",
        "exd_display_type|0=numerical,1=simple,2=graph,3=bar,4=circle_graph,5=virtual_partner,6=balance,7=string_list,8=string,9=simple_dynamic_icon,10=gauge,",
        "exd_data_units|0=no_units,1=laps,2=miles_per_hour,3=kilometers_per_hour,4=feet_per_hour,5=meters_per_hour,6=degrees_celsius,7=degrees_farenheit,8=zone,9=gear,10=rpm,11=bpm,12=degrees,13=millimeters,14=meters,15=kilometers,16=feet,17=yards,18=kilofeet,19=miles,20=time,21=enum_turn_type,22=percent,23=watts,24=watts_per_kilogram,25=enum_battery_status,26=enum_bike_light_beam_angle_mode,27=enum_bike_light_battery_status,28=enum_bike_light_network_config_type,29=lights,30=seconds,31=minutes,32=hours,33=calories,34=kilojoules,35=milliseconds,36=second_per_mile,37=second_per_kilometer,38=centimeter,39=enum_course_point,40=bradians,41=enum_sport,42=inches_hg,43=mm_hg,44=mbars,45=hecto_pascals,46=feet_per_min,47=meters_per_min,48=meters_per_sec,49=eight_cardinal,",
        "exd_qualifiers|0=no_qualifier,1=instantaneous,2=average,3=lap,4=maximum,5=maximum_average,6=maximum_lap,7=last_lap,8=average_lap,9=to_destination,10=to_go,11=to_next,12=next_course_point,13=total,14=three_second_average,15=ten_second_average,16=thirty_second_average,17=percent_maximum,18=percent_maximum_average,19=lap_percent_maximum,20=elapsed,21=sunrise,22=sunset,23=compared_to_virtual_partner,24=maximum_24h,25=minimum_24h,26=minimum,27=first,28=second,29=third,30=shifter,31=last_sport,32=moving,33=stopped,34=estimated_total,242=zone_9,243=zone_8,244=zone_7,245=zone_6,246=zone_5,247=zone_4,248=zone_3,249=zone_2,250=zone_1,",
        "exd_descriptors|0=bike_light_battery_status,1=beam_angle_status,2=batery_level,3=light_network_mode,4=number_lights_connected,5=cadence,6=distance,7=estimated_time_of_arrival,8=heading,9=time,10=battery_level,11=trainer_resistance,12=trainer_target_power,13=time_seated,14=time_standing,15=elevation,16=grade,17=ascent,18=descent,19=vertical_speed,20=di2_battery_level,21=front_gear,22=rear_gear,23=gear_ratio,24=heart_rate,25=heart_rate_zone,26=time_in_heart_rate_zone,27=heart_rate_reserve,28=calories,29=gps_accuracy,30=gps_signal_strength,31=temperature,32=time_of_day,33=balance,34=pedal_smoothness,35=power,36=functional_threshold_power,37=intensity_factor,38=work,39=power_ratio,40=normalized_power,41=training_stress_Score,42=time_on_zone,43=speed,44=laps,45=reps,46=workout_step,47=course_distance,48=navigation_distance,49=course_estimated_time_of_arrival,50=navigation_estimated_time_of_arrival,51=course_time,52=navigation_time,53=course_heading,54=navigation_heading,55=power_zone,56=torque_effectiveness,57=timer_time,58=power_weight_ratio,59=left_platform_center_offset,60=right_platform_center_offset,61=left_power_phase_start_angle,62=right_power_phase_start_angle,63=left_power_phase_finish_angle,64=right_power_phase_finish_angle,65=gears,66=pace,67=training_effect,68=vertical_oscillation,69=vertical_ratio,70=ground_contact_time,71=left_ground_contact_time_balance,72=right_ground_contact_time_balance,73=stride_length,74=running_cadence,75=performance_condition,76=course_type,77=time_in_power_zone,78=navigation_turn,79=course_location,80=navigation_location,81=compass,82=gear_combo,83=muscle_oxygen,84=icon,85=compass_heading,86=gps_heading,87=gps_elevation,88=anaerobic_training_effect,89=course,90=off_course,91=glide_ratio,92=vertical_distance,93=vmg,94=ambient_pressure,95=pressure,96=vam,",
        "auto_activity_detect|0x00000000=none,0x00000001=running,0x00000002=cycling,0x00000004=swimming,0x00000008=walking,0x00000020=elliptical,0x00000400=sedentary,",
        "supported_exd_screen_layouts|0x00000001=full_screen,0x00000002=half_vertical,0x00000004=half_horizontal,0x00000008=half_vertical_right_split,0x00000010=half_horizontal_bottom_split,0x00000020=full_quarter_split,0x00000040=half_vertical_left_split,0x00000080=half_horizontal_top_split,",
        "fit_base_type|0=enum,1=sint8,2=uint8,131=sint16,132=uint16,133=sint32,134=uint32,7=string,136=float32,137=float64,10=uint8z,139=uint16z,140=uint32z,13=byte,142=sint64,143=uint64,144=uint64z,",
        "turn_type|0=arriving_idx,1=arriving_left_idx,2=arriving_right_idx,3=arriving_via_idx,4=arriving_via_left_idx,5=arriving_via_right_idx,6=bear_keep_left_idx,7=bear_keep_right_idx,8=continue_idx,9=exit_left_idx,10=exit_right_idx,11=ferry_idx,12=roundabout_45_idx,13=roundabout_90_idx,14=roundabout_135_idx,15=roundabout_180_idx,16=roundabout_225_idx,17=roundabout_270_idx,18=roundabout_315_idx,19=roundabout_360_idx,20=roundabout_neg_45_idx,21=roundabout_neg_90_idx,22=roundabout_neg_135_idx,23=roundabout_neg_180_idx,24=roundabout_neg_225_idx,25=roundabout_neg_270_idx,26=roundabout_neg_315_idx,27=roundabout_neg_360_idx,28=roundabout_generic_idx,29=roundabout_neg_generic_idx,30=sharp_turn_left_idx,31=sharp_turn_right_idx,32=turn_left_idx,33=turn_right_idx,34=uturn_left_idx,35=uturn_right_idx,36=icon_inv_idx,37=icon_idx_cnt,",
